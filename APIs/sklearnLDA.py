...
class sklearnLDA_NMF():
    def __init__(self, model, df_reviews, ld, alpha, eta,epochs, lm, lo, bs, num_topics, num_words, random_state, ngram_range,\
        ...
        if model == "lda":
            self.model = LatentDirichletAllocation(n_components=num_topics, max_iter=epochs,learning_decay=ld, \
                                            doc_topic_prior=alpha, learning_method= lm ,learning_offset=lo,\
                                            evaluate_every = -1,batch_size=bs, random_state=random_state)
        elif model == "nmf":
            self.model = NMF(n_components=num_topics, random_state = random_state,\
                alpha = alpha, l1_ratio = 0.5, init = 'nndsvd', max_iter = epochs)
